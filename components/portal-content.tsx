...
function scoreText(value: string | number | undefined | null, suffix = "") {
...
async function candidateContent(slug: string) {
  ...
  const [jobs, profile, notifications, interviews] = await Promise.all([
    fetchJobs(),
  ...
  switch (slug) {
    case "profile":
      return {
        ...
        body: <section className="grid gap-6 xl:grid-cols-2"><div className="panel p-6"><h2 className="text-xl font-semibold text-ink">Personal information</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><Input defaultValue={profile?.candidate.full_name.split(" ")[0] ?? "Ava"} /><Input defaultValue={profile?.candidate.full_name.split(" ").slice(1).join(" ") || "Morgan"} /><Input defaultValue={profile?.candidate.email ?? "ava@example.com"} /><Input defaultValue={primaryApplication?.phone ?? "+94 77 123 4567"} /></div><h3 className="mt-8 text-lg font-semibold text-ink">AI summary</h3><div className="mt-4 space-y-3">{profileRows(profile).map(([label, value]) => <div key={label} className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-700"><span className="font-semibold text-ink">{label}:</span> {value}</div>)}</div></div><div className="panel p-6"><h2 className="text-xl font-semibold text-ink">Experience summary</h2><div className="mt-5 space-y-4"><TextArea defaultValue="Product leader with experience designing enterprise SaaS workflows and cross-functional operating models." /><TextArea defaultValue={`Active application: ${primaryApplication?.role ?? "AI Product Designer"}. Status: ${primaryApplication?.status ?? "Hiring review"}.`} /><button className="rounded-full bg-brand px-5 py-3 text-sm font-semibold text-white">Save profile</button></div></div></section>
      };
    case "apply":
      return {
    ...
    case "notifications":
      return {
        ...
        body: <section className="grid gap-4">{notifications.length ? notifications.map((item) => <div key={`${item.tag}-${item.title}`} className="panel p-6"><h2 className="text-lg font-semibold text-ink">{item.title}</h2><p className="mt-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary">{item.tag}</p><p className="mt-3 text-sm leading-7 text-muted">{item.body}</p></div>) : <div className="panel p-6 text-sm text-muted">No notifications yet.</div>}</section>
      };
    case "interviews":
      return {
    ...
    default:
      return {
        ...
        body: <section className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]"><div className="panel p-6"><h2 className="text-xl font-semibold text-ink">Application progress</h2><div className="mt-6">{primaryApplication ? <Timeline items={primaryApplication.timeline} /> : <p className="text-sm text-muted">No active applications yet.</p>}</div></div><div className="space-y-6"><ChartCard title="Profile readiness" subtitle="A strong profile improves job recommendations and screening accuracy." values={[{ label: "Match score", value: profile?.cv_matching?.matching_score ?? 0 }, { label: "Soft skills", value: profile?.soft_skills?.overall_score ?? 0 }, { label: "Final score", value: profile?.final_score?.composite_score ?? 0 }]} /><div className="panel p-6"><h2 className="text-xl font-semibold text-ink">Recommended jobs</h2><div className="mt-4 space-y-4">{jobs.slice(0, 3).map((job) => <div key={job.id} className="rounded-2xl bg-slate-50 p-4"><p className="font-semibold text-ink">{job.title}</p><p className="mt-2 text-sm text-muted">{job.summary}</p></div>)}</div></div></div></section>
      };
  }
}

async function adminContent(slug: string) {
  const [jobs, applications, summary, profile] = await Promise.all([
    fetchJobs(),
    fetchApplications(),
    fetchDashboardSummary(),
    fetchCandidateProfile(1)
  ]);
  ...
  switch (slug) {
    ...
    case "candidates/review":
      return {
        ...
        body: <section className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]"><div className="space-y-6"><div className="panel p-6"><h2 className="text-xl font-semibold text-ink">Candidate profile summary</h2><p className="mt-4 text-sm leading-7 text-muted">{profile?.candidate.full_name ?? "Candidate"} is under review for {applications[0]?.role ?? "the selected role"}.</p></div><div className="panel p-6"><h2 className="text-xl font-semibold text-ink">AI score breakdown</h2><div className="mt-4 space-y-3 text-sm text-muted">{profileRows(profile).map(([label, value]) => <p key={label}>{label}: {value}</p>)}</div></div></div><div className="grid gap-6"><div className="panel p-6"><h2 className="text-xl font-semibold text-ink">Structured CV</h2><p className="mt-4 text-sm leading-7 text-muted">Backend candidate profile is connected. Replace the in-memory store with database-backed structured CV fields next.</p></div><div className="panel p-6"><h2 className="text-xl font-semibold text-ink">Interview responses & evidence</h2><p className="mt-4 text-sm leading-7 text-muted">Live profile and application endpoints are connected. Evidence playback can now be wired to stored interview sessions and screenshots.</p></div><div className="flex flex-wrap gap-3">{["Reject", "Shortlist", "Select", "Physical interview", "Direct join"].map((action) => <button key={action} className="rounded-full bg-brand px-5 py-3 text-sm font-semibold text-white">{action}</button>)}</div></div></section>
      };
    case "interviews":
      return {
    ...
    case "communications":
      return {
        ...
        body: <section className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]"><form className="panel p-6"><h2 className="text-xl font-semibold text-ink">Trigger communication</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><Input placeholder="Candidate name" /><Select defaultValue="Select template"><option>Select template</option><option>Rejection email</option><option>Selection email</option><option>Physical interview invite</option></Select></div><div className="mt-4"><TextArea placeholder="Customize the message before sending" /></div><button className="mt-6 rounded-full bg-brand px-6 py-3 text-sm font-semibold text-white">Send communication</button></form><DataTable headers={["Recipient", "Template", "Status", "Time"]} rows={applications.map((application) => [application.candidate, "Status update", application.status, "Live backend"]).slice(0, 3)} /></section>
      };
    case "reports":
      return {
...
function hiringContent(slug: string, applications: Application[]) {
  switch (slug) {
    ...
    case "interviews":
      return { title: "Interview Review", description: "Watch recordings, inspect answers, and review AI-generated insights.", body: <VideoInterviewPanel /> };
    case "comparison":
    ...
    default:
      return { title: "Hiring Dashboard", description: "Access assigned roles, shortlisted candidates, and review tasks.", body: <section className="grid gap-6 xl:grid-cols-[1fr_0.9fr]"><ChartCard title="Assigned role load" subtitle="Current review capacity across your queue." values={[{ label: "Candidate queue", value: applications.length || 0 }, { label: "Shortlisted", value: applications.filter((item) => item.status.toLowerCase().includes("hiring") || item.status.toLowerCase().includes("short")).length }, { label: "Review ready", value: applications.filter((item) => item.match_score !== "Pending").length }]} /><div className="panel p-6"><h2 className="text-xl font-semibold text-ink">Reviewer pulse</h2><div className="mt-4 space-y-4">{reviewers.map((reviewer) => <div key={reviewer.name} className="rounded-2xl border border-border p-4"><p className="font-semibold text-ink">{reviewer.name}</p><p className="mt-1 text-sm text-muted">{reviewer.role}</p><p className="mt-2 text-sm text-primary">{reviewer.status}</p></div>)}</div></div></section> };
  }
}
